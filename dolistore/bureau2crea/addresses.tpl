...
<div class="addresses">
	...
	<ul class="address {if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if}">
		...
		<li class="address_update"><a href="{$base_dir_ssl}address.php?id_address={$address.id_address|intval}" title="{l s='Update'}" class="orange_btn">{l s='Update'}</a></li>
		<li class="address_delete"><a href="{$base_dir_ssl}address.php?id_address={$address.id_address|intval}&amp;delete" onclick="return confirm('{l s='Are you sure?'}');" title="{l s='Delete'}" class="back_btn">{l s='Delete'}</a></li>
	</ul>
	{/foreach}
	<p class="clear" />
</div>
...
<ul class="footer_links">
	<li><a href="{$base_dir_ssl}my-account.php" class="back_btn">{l s='Back to Your Account'}</a></li>
	<li><a href="{$base_dir}" class="orange_btn">{l s='Home'}</a></li>
</ul>