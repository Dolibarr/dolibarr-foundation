	...
	if ($_GET["up"] == 1) {
		if ($_FILES['image_product']['error']) {
			...
			//if (! in_array($extention_image, array("jpg","jpeg"))) {
				echo "<div style='color:#FF0000'>"; echo aff("Votre image n'est pas en .jpg, le format .".$extention_image." n'est pas autorise.",
			...
			if ($flagError == 0) {
				...
				if (move_uploaded_file($_FILES['image_product']['tmp_name'], $chemin_destination) != true) {
					...
					echo "<script>alert('";
					echo aff("L image a été liée au module/produit.", "Image has been linked to module/product.", $iso_langue_en_cours);
					echo "'); </script>";
...
<table width="100%" border="0" cellspacing="10" cellpadding="0">
 ...
 <tr>
  ...
  <tr>
    <td nowrap="nowrap" valign="top"><?php echo aff("Ajoutez une nouvelle image<br>à ce produit : ", "Add a new image<br>to this product : ", $iso_langue_en_cours); ?></td>
    <td nowrap="nowrap">
...
<table width="100%" border="0" cellspacing="10" cellpadding="0">
   ...
   $query = 'SELECT `id_image` FROM `'._DB_PREFIX_.'image`
	...
	if (sizeof($result) > 0)
	{
		?>
		  <tr>
			<td nowrap="nowrap" valign="top" colspan="2"><?php echo aff("Images de ce produit : ", "Product's pictures : ", $iso_langue_en_cours); ?></td>
		  </tr>
		<?php
		foreach ($result AS $row)
		{
	...
	//else
	//{
			//echo aff("Aucune pour le moment (Cliquer sur Soumettre image) : ", "None for the moment (Click on submit now) : ", $iso_langue_en_cours);
	//}
...
<table width="100%" border="0" cellspacing="10" cellpadding="0">
  <tr>
    <td nowrap="nowrap" valign="top" align="center" colspan="2">
        ...
        <button <?php print sizeof($result)?'':'disabled="true"'; ?> type="button" onclick="JavaScript: alert('<?php echo aff("Votre module/produit a été mis à jour", "Module/product updated", $iso_langue_en_cours); ?>'); window.location='./my-sales-manage-product.php';">
	        <?php echo aff("Terminer", "Finish", $iso_langue_en_cours); ?>
		</button>
...
{
	echo aff("<div style='color:#FF0000'>Vous n'etes pas le proprietaire de ce produit<br> </div>", "<div style='color:#FF0000'>This product is not yours<br> </div>", $iso_langue_en_cours);
}
