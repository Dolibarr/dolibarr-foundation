...
"Statistics are for download/sells since the last payment received ".($datelastpayment?"(<b>".date('Y-m-d',$datelastpayment)."</b>)":"")." for your sells of components submited by for current user (<b>".$publisher.($company?" - ".$company:"")."</b>)",
$iso_langue_en_cours);
...
if ($totalamount > 0)
{
    ...
    if ($socid)
    {
        ...
        $WS_METHOD  = 'getSuppliersInvoicesForThirdParty';
        prestalog("Create soapclient_nusoap for URL=".$WS_DOL_URL);
        $soapclient = new soapclient_nusoap($WS_DOL_URL);
        if ($soapclient)
        {
