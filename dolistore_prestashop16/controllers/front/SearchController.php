...
class SearchControllerCore extends FrontController
{
    ...
    public function initContent()
    {
        ...
        if ($this->instant_search && !is_array($query)) {
        ...
        }elseif (($query = Tools::getValue('search_query', Tools::getValue('ref'))) && !is_array($query) && ($tag = urldecode(Tools::getValue('tag'))) && !is_array($tag)) {
            $this->productSort();
            $this->n = abs((int)(Tools::getValue('n', $product_per_page)));
            $this->p = abs((int)(Tools::getValue('p', 1)));
            ...
            //$id_lang, $expr, $page_number = 1, $page_size = 1, $order_by = 'position', $order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null, $tags
            $search = Search::findWithTag($this->context->language->id, $query, $this->p, $this->n, $this->orderBy, $this->orderWay,false,true,null,$tag);
            if (is_array($search['result'])) {
                foreach ($search['result'] as &$product) {
                    $product['link'] .= (strpos($product['link'], '?') === false ? '?' : '&').'search_query='.urlencode($query).'&results='.(int)$search['total'].'&tag='.$tag;
                }
            }
