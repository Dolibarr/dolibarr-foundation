...
public static function findWithTag($id_lang, $expr, $page_number = 1, $page_size = 1, $order_by = 'position',
        $order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null, $tags)
    {
        if (!$context) {
        ...
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity,
			 ...
			 image_shop.`id_image` id_image, il.`legend`, m.`name` manufacturer_name '.$score.',
				...
				INNER JOIN '._DB_PREFIX_.'product_tag as pt ON (pt.id_product = pl.id_product)
				INNER JOIN '._DB_PREFIX_.'tag as tg ON (pt.id_tag = tg.id_tag AND tg.`id_lang` = '.(int)$id_lang.')
				WHERE tg.name = "'.pSQL($tags).'" AND p.`id_product` '.$product_pool.'
