...
if (!isset($argv[1]) || !isset($argv[2]) || !isset($argv[3]) || !isset($argv[4]) || !isset($argv[5])) {	// Check parameters
	print "Usage: " . $script_file . " db_host db_name db_user db_password db_port source_dir\n";
...
SELECT
	ppd.id_product,
	ppd.filename,
	ppd.display_filename 
FROM ps_product_download ppd ";
...
print "Import products attached files - May take a long time...\n";
...
if ($result_products = $conn->query($products_query)) {
	...
	while ($obj = $result_products->fetch_object()) {
		...
		$result = $product->fetch(0, '', $obj->id_product);

		if ($result < 0) {
			$error_messages[] = " - Error in fetch product with ref_ext => " . $obj->id_product . " result code = " . $result . " - " . $product->errorsToString();
			$error++;
		} else {

			$upload_dir = $conf->product->multidir_output[1] . '/' . get_exdir(0, 0, 0, 1, $product, 'product');

			$url = $source_dir.$obj->filename;

			// check if file exists
			if (!file_exists($url)) {
				print " - File not found for product with ref_ext => " . $obj->id_product . "\n";
				$error_messages[] = " - File not found for product with ref_ext => " . $obj->id_product;
				continue;
			}
			...
			$package = $upload_dir . '/' . basename($obj->display_filename);

			if (copy($url, $package) != true){
				$error_messages[] = " - Error in copy package file with ref_ext => " . $obj->id_product;
				$error++;
			} else {
				// Add index to db
				$filename = basename($obj->display_filename);
				addFileIntoDatabaseIndex($upload_dir, $filename, 'MARKETPLACE', 'uploaded', 1, $product);
				...
				$ecmfile->position = 100;
				$result_update_ecmfile = $ecmfile->update($user);

				if ($result_update_ecmfile < 0) {
					$error_messages[] = " - Error in copy Package product with ref_ext => " . $obj->id_product . " result code = " . $result_update_ecmfile . " - " . $ecmfile->errorsToString();
					$error++;
				} else {
					print " - Copy package product with ref_ext => " . $obj->id_product . " ok \n";
					$count_imported++;
				}
			}
...
if (!$error) {
	$db->commit();
	print $count_imported . ' packages imported' . "\n";
	print '--- end ok' . "\n";
} else {
