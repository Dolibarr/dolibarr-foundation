...
$sql  = "select s.rowid, s.ref_ext as id_customer, s.nom as company, s.import_key, s.tva_intra, s.address, s.zip, s.town, s.email ";
$sql .= "from ".MAIN_DB_PREFIX."societe s ";
$sql .= "where s.ref_ext is not null";
$sql .= " and s.ref_ext != ''";
$sql .= " and s.import_key = '20250102133844'";
$sql .= " and (s.tva_intra is null or s.tva_intra = '')";
//print $query = $sql . "\n";
//exit;
...
if ($resultSql = $db->query($sql)) {
	while ($obj = $resultSql->fetch_object()) {
		// Get VAT number from external system
		$vatSql  = "SELECT vat_number, company, address1, postcode, city, ";
		$vatSql .= "(CASE WHEN company = '" . $db->escape($obj->company) . "' THEN 1 ELSE 0 END + ";
		$vatSql .= "CASE WHEN address1 = '" . $db->escape($obj->address) . "' THEN 1 ELSE 0 END + ";
		$vatSql .= "CASE WHEN postcode = '" . $db->escape($obj->zip) . "' THEN 1 ELSE 0 END + ";
		$vatSql .= "CASE WHEN city = '" . $db->escape($obj->town) . "' THEN 1 ELSE 0 END) AS match_score ";
		$vatSql .= "FROM ps_address ";
		$vatSql .= "WHERE id_customer = " . (int)$obj->id_customer . " ";
		$vatSql .= "AND active = 1 ";
		$vatSql .= "AND deleted = 0 ";
		$vatSql .= "AND vat_number IS NOT NULL ";
		$vatSql .= "AND vat_number != '' ";
		$vatSql .= "ORDER BY match_score DESC, vat_number ASC LIMIT 1";

		$resultVat = $conn->query($vatSql);
		if ($resultVat && $rowVat = $conn->fetch_object($resultVat)) {
			// Update VAT number in Dolibarr
			$obj->tva_intra 	= $rowVat->vat_number;

			$updateSql 			= "UPDATE " . MAIN_DB_PREFIX . "societe ";
			$updateSql 			.= "SET tva_intra = '" . $db->escape($obj->tva_intra) . "' ";
