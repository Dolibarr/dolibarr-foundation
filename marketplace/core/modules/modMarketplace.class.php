...
class modMarketplace extends DolibarrModules
{
	...
	public function init($options = '')
	{
		...
		$result10=$extrafields->addExtraField('marketplace_validity_duration',   "MarketplaceValidityDuration", 'int', 110,  12, 'product',   0, 0, '', '', 1, '', -1, 0, '', '', 'marketplace@marketplace', 'isModEnabled("marketplace")');
		$result11=$extrafields->addExtraField('marketplace_pricecomment',        "PriceComment", 'varchar', 120,  128, 'product',   0, 0, '', '', 1, '', -1, 0, '', '', 'marketplace@marketplace', 'isModEnabled("marketplace")');
		$result12=$extrafields->addExtraField('marketplace_disclaimer_message',  "DisclaimerMessage", 'varchar', 130,  255, 'product',   0, 0, '', '', 1, '', -1, 0, '', '', 'marketplace@marketplace', 'isModEnabled("marketplace")');
		...
		*/
		...
		if ($id_template <= 0) {
			...
			$email_sql = "INSERT INTO ".MAIN_DB_PREFIX."c_email_templates (label, lang, module, type_template, fk_user, private, position, topic, email_from, joinfiles, defaultfortype, content, entity, active, enabled) VALUES ('(resetPasswordMarketplace)', '', 'marketplace', 'thirdparty', null, 0, 120, '[__[MARKETPLACE_NAME]__] __(passwordResetRequest)__ (IP __USER_REMOTE_IP__)', null, 0, 0, '" . $email_content . "' , ".((int) $conf->entity).", 1, 1);";
			$result = $this->db->query($email_sql);
			if ($result) {
				$id_template = $this->db->last_insert_id(MAIN_DB_PREFIX."c_email_templates");
			} else {
